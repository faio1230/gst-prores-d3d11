// Planar 422/444 10/12-bit D3D11Memory -> full-range RGB10A2.
// Read the actual low-bit integer code before applying the BT.709 matrix.
...
cbuffer Parameters : register(b0) {
    uint output_width;
    uint output_height;
    uint mode; // low bit: chroma horizontal shift; bits 8..15: input depth
    uint reserved;
};
...
void main(uint3 id : SV_DispatchThreadID) {
    ...
    float cr = (cr_code / depth_scale - 512.0) / 896.0;
