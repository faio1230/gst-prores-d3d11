# PTS同期を保った標準sinkで、専用RGBとsinkの間のqueueを交互比較する診断。
[CmdletBinding()]
param(
    [int]$Repeats = 2,
    [int]$Loops = 3,
    [int]$SettleMs = 150,
    [string]$OutDir = 'results/postrgb-queue-presentmon-2026-09-24',
...
$sessionPrefix = 'PQr' + (Get-Date -Format 'MMddHHmmss')
for ($repeat = 0; $repeat -lt $Repeats; ++$repeat) {
    $conditions = if ($repeat % 2 -eq 0) { @('direct', 'queue') } else { @('queue', 'direct') }
    foreach ($condition in $conditions) {
        $tag = "r$repeat-$condition"
        ...
        New-Item -ItemType Directory -Force $trialDir | Out-Null
        ...
        $session = "$sessionPrefix$repeat$($condition.Substring(0,1))"
        ...
        try {
            ...
            $arguments = @('scripts/benchmark-d3d11-display.py', $sourcePath,
                ...
                '--trace-sink-return', '--settle-ms', [string]$SettleMs, '--out', $trialDir)
            if ($condition -eq 'queue') { $arguments += '--queue-after-rgb' }
            ...
            if ($metrics.postrgb_queue -ne ($condition -eq 'queue') -or
                !$metrics.sink_clock_sync -or $metrics.present_sync_interval -ne 0) {
            ...
            if ($condition -eq 'queue' -and
                ($metrics.stages.rgb_dequeued -ne $metrics.stages.rgb -or
                 $metrics.stages.missing_in_postrgb_queue_count -ne 0)) {
                throw "RGB後queueのフレーム保持に失敗: $tag"
            }
            $completed = $true
            Write-Host "$tag`: rendered=$($metrics.rendered)/$($metrics.source_frames * $Loops), QoS=$($metrics.qos_messages), OS未表示=$($record.interior_captured_but_not_displayed_aligned)/$($record.interior_source_frames_aligned), 最大表示間隔=$($record.max_display_gap_ms)ms"
        } finally {
            if (!$completed) {
