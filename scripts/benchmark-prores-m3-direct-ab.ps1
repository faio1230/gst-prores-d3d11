# M2採用版とM3作業版のHQ directを同一素材・交互4組で比較する。
[CmdletBinding()]
param([string]$OutDir = 'results/prores-m3-2026-09-24/hq-direct-ab')
$ErrorActionPreference = 'Stop'
$root = Split-Path $PSScriptRoot -Parent
$build = Join-Path $root 'build/vs18'
$oldBuild = Join-Path $build 'prores-m2-direct-ab-new'
$newBuild = Join-Path $build 'prores-m3-direct-ab-new'
$newSource = Join-Path $build 'plugins/Release'
$out = Join-Path $root $OutDir
$oldNames = @('gstproresd3d11.dll','prores_vld.cso','prores_idct_unorm.cso','prores_rgb.cso')
$newNames = $oldNames + @('prores_alpha.cso','prores_pack_alpha.cso','prores_rgb_alpha.cso')
$sources = @((Join-Path $root 'media/synthetic-1080p60-hq.mov'),
             (Join-Path $root 'media/synthetic-2160p60-hq.mov'))
$m2 = Get-Content -LiteralPath (Join-Path $root 'results/prores-m2-2026-09-24/hq-direct-ab/summary.json') -Raw |
    ConvertFrom-Json
if ((Test-Path -LiteralPath $out) -or (Test-Path -LiteralPath $newBuild)) {
    throw '既存のM3結果・配置は上書きしません'
}
foreach ($path in $sources + @($oldNames | ForEach-Object { Join-Path $oldBuild "plugins/Release/$_" }) +
    @($newNames | ForEach-Object { Join-Path $newSource $_ }) +
...
if ($oldHash -ne $m2.new_plugin_sha256 -or !$m2.results -or
    @($m2.results | Where-Object { !$_.passed }).Count) {
    throw 'M2採用版の由来を確認できません'
}
...
foreach ($name in $newNames) { Copy-Item -LiteralPath (Join-Path $newSource $name) -Destination $newPlugin }
...
[pscustomobject]@{
    pairs=4; old_source='M2採用版（prores-m2-direct-ab-new）'
...
if (@($summary | Where-Object { !$_.passed }).Count) {
    throw 'M3 HQ direct性能ゲート未達。summary.jsonを参照'
}
