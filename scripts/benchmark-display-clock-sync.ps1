# 標準D3D11 sinkの時計同期あり／なしを診断用Present1(1)で交互比較する。
[CmdletBinding()]
param(
    [int]$Repeats = 2,
    [int]$Loops = 1,
    [int]$SettleMs = 150,
    [switch]$Topmost,
    [string]$OutDir = 'results/display-clock-sync-2026-09-24',
...
$sessionPrefix = 'PCl' + (Get-Date -Format 'MMddHHmmss')
for ($repeat = 0; $repeat -lt $Repeats; ++$repeat) {
    $conditions = if ($repeat % 2 -eq 0) { @('clock', 'present') } else { @('present', 'clock') }
    foreach ($condition in $conditions) {
        ...
        try {
            ...
            $arguments = @('scripts/benchmark-d3d11-display.py', $sourcePath,
                ...
                '--trace-sink-return', '--present-sync-interval', '1', '--settle-ms', [string]$SettleMs,
                '--out', $trialDir)
            if ($Topmost) { $arguments += '--topmost-window' }
            if ($condition -eq 'present') { $arguments += '--sink-no-clock-sync' }
            ...
            $presentRows = @(Import-Csv -LiteralPath $capture)
            if (!$presentRows.Count -or @($presentRows | Where-Object {
                $_.SyncInterval -ne '1' -or $_.PresentFlags -ne '0'
            }).Count) {
                throw "PresentMonの同期値またはflagsが不一致: $tag"
            }
            ...
            if ($metrics.sink_clock_sync -ne ($condition -eq 'clock') -or
                $metrics.present_sync_interval -ne 1) {
                throw "sink同期設定の記録が不一致: $tag"
            }
            ...
            $completed = $true
            Write-Host "$tag`: rendered=$($metrics.rendered) dropped=$($metrics.dropped) summary=$summary"
        } finally {
            if (!$completed) {
